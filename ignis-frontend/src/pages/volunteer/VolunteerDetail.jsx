...
const maskEmail = (email) => {
  ...
  const maskedLocal =
    local.slice(0, keep) + '*'.repeat(Math.max(1, local.length - keep));
...
const toImageUrl = (p) => {
  if (!p || String(p).trim() === '' || String(p).toLowerCase() === 'null')
    return testImage;
...
/* ================= 유틸: 지도 오픈 ================= */
/** 장소 클릭 시 카카오맵 검색 페이지를 새 탭에서 오픈 */
const openMap = (address) => {
  if (!address || address === '장소 미정') return;
...
export default function VolunteerDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const res = await fetch(`/volunteer/react/detail/${id}`, {
          credentials: 'include',
        });
        ...
        if (!ignore && payload?.volunteerId) {
          const check = await fetch(
            `/volunteer/${payload.volunteerId}/join/me`,
            { credentials: 'include' }
          );
          if (check.ok) {
    ...
    (async () => {
      try {
        const candidates = [
          `/volunteer/react/related?volunteerId=${encodeURIComponent(
            id
          )}&limit=6`,
        ...
        for (const url of candidates) {
          try {
            ...
            const arr = Array.isArray(data)
              ...
              ? data.items
              : Array.isArray(data?.volunteerList)
              ? data.volunteerList
              : [];
            const sorted = [...arr].sort(
              (a, b) =>
                Number(b.viewCount ?? b.views ?? 0) -
                Number(a.viewCount ?? a.views ?? 0)
            );
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`/volunteer/api/${id}/like/state`, {
          credentials: 'include',
        });
        const out = await res.json().catch(() => ({}));
        if (out?.result === 'success' || typeof out?.likeCount !== 'undefined') {
          setLiked(!!out.liked);
  ...
  useViewOnce({
    ...
    onUpdated: (views) =>
      setVol((p) => (p ? { ...p, viewCount: views, views } : p)),
  });
  ...
  const progress = useMemo(
    () =>
      target
        ? Math.min(100, Math.floor((current * 100) / target))
        : 0,
    [current, target]
  );
  ...
  const toggleJoin = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const updated = { ...vol };
      updated.currentPeople = Math.max(
        0,
        (updated.currentPeople || 0) + (joined ? -1 : 1)
      );
      setVol(updated);
      setJoined(!joined);
      message.success(
        joined ? '봉사 참여가 취소되었습니다.' : '봉사에 참여했습니다.'
      );
    } catch {
  ...
  const fetchParticipants = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/volunteer/${vol.volunteerId}/participants`,
        { credentials: 'include' }
      );
      if (res.status === 403) {
  ...
  if (loading) {
    return (
      <Layout>
        <div
          className="volunteer-main-section"
          style={{
            padding: 40,
            display: 'flex',
              ...
              items={[
                {
                  ...
                  children: (
                    <>
                      {/* 상세 설명 */}
                      <Card
                        className="volunteer-content-card"
                        variant="bordered"
                      >
                      ...
                      {/* 주의사항 */}
                      <Card
                        className="volunteer-warning-card"
                        variant="bordered"
                      >
                        <Title
                          level={5}
                          className="volunteer-warning-title"
                        >
                          참여 전 꼭 확인해주세요
                        </Title>
                        <ul className="volunteer-warning-list">
                          <li>
                            신청 후 무단 불참은 다른 참여자에게 피해가
                            됩니다.
                          </li>
                          <li>
                            활동 장소·시간은 주최 측 사정으로 일부
                            변경될 수 있습니다.
                          </li>
                          <li>
                            개인정보는 참여 목적 외로 사용되지 않습니다.
                          </li>
                          <li>
                            봉사 인증서 발급 여부는 주최 측 정책에
                            따릅니다.
                          </li>
                        </ul>
                      </Card>
            ...
            <Card className="volunteer-info-card" variant="bordered">
              ...
              <div className="volunteer-stacked-metrics">
                ...
                <div className="volunteer-metric">
                  ...
                  <Text strong className="volunteer-accent">
                    {progress}%
                  </Text>
                </div>
                <div className="volunteer-metric">
