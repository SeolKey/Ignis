        ...
        <Row gutter={[24, 24]}>
            ...
            <Card bordered={false} className="thumbnail-card">
              <Carousel autoplay autoplaySpeed={3000} pauseOnHover={false} dots>
                {imagesForCarousel.map((src, idx) => (
                  <div key={idx}>
                    <img src={src || testImage} alt={`이미지-${idx}`} className="thumbnail-image" style={{ height: 300 }} onError={(e) => { e.currentTarget.src = testImage; }} />
                  </div>
                ))}
              </Carousel>
            </Card>
            ...
            <Tabs defaultActiveKey="1" className="custom-tabs">
              <TabPane tab="상세내용" key="1">
                ...
                <Card className="content-card" bordered={false}>
                  <Paragraph style={{ whiteSpace: 'pre-wrap' }}>{vol?.description || '봉사 설명이 등록되지 않았습니다.'}</Paragraph>
                </Card>
                <Paragraph style={{ marginTop: 24 }}>장소</Paragraph>
                <Card className="content-card" bordered={false}>
                  <Paragraph><EnvironmentOutlined style={{ marginRight: 8 }} /><Text>{loc}</Text></Paragraph>
                </Card>
              </TabPane>
          ...
          <Col xs={24} md={8}>
            <Card className="info-card" variant="borderless">
              ...
              <div className="project-period"><CalendarOutlined style={{ marginRight: 8 }} /><Text>{startStr}{endStr ? ` ~ ${endStr}` : ''}</Text></div>
              {loc && (<div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><EnvironmentOutlined /><Text>{loc}</Text></div>)}
              <Divider style={{ margin: '16px 0' }} />
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text strong>참여 현황</Text>
                <Text type="secondary"><TeamOutlined style={{ marginRight: 6 }} />{Number(vol?.currentPeople || 0)} / {Number(vol?.maxParticipants || 0)}</Text>
              </div>
              <Progress percent={progress} showInfo={false} status="active" />
              {vol?.status && (<div style={{ marginTop: 8 }}><CheckCircleTwoTone twoToneColor="#52c41a" style={{ marginRight: 6 }} /><Text>상태: {vol.status}</Text></div>)}

              <Button type={joined ? 'default' : 'primary'} danger={joined} block style={{ marginBottom: 12 }} onClick={toggleJoin}>
                {joined ? '참여 취소하기' : '봉사 참여하기'}
              </Button>
              <Button icon={<ShareAltOutlined />} block style={{ marginTop: 12 }} onClick={() => navigator.clipboard.writeText(window.location.href).then(()=>message.success('링크 복사됨'))}>
                공유하기
              </Button>
            </Card>
          </Col>
